package TbCtrlOsReal;

// Exercise the Controller startOsReal() path end-to-end: preload the
// WeightSRAM and ActivationSRAM with a small 4x4 @ 4x4 OS dispatch, fire
// startOsReal(kLen=4), wait for Done, and check the full psum matrix via
// resultsMatrix().
//
...
module mkTbCtrlOsReal();
   ...
   rule dispatch_os_real (cycle == 5 && phase == 0);
      ctrl.startOsReal(0, 0, 4);
      phase <= 1;
      $display("Cycle %0d: dispatch startOsReal(kLen=4)", cycle);
   endrule

   rule finish (phase == 1 && ctrl.isDone);
      let m = ctrl.resultsMatrix;
      let mode_ok = (ctrl.getDataflowMode == DF_OUTPUT_STATIONARY_REAL);
      Bool all_ok = True;
      ...
      $display("Cycle %0d: OS-real resultsMatrix =", cycle);
      for (Integer r = 0; r < 4; r = r + 1)
         $display("  [%0d, %0d, %0d, %0d]", m[r][0], m[r][1], m[r][2], m[r][3]);
      if (all_ok && mode_ok) begin
         $display("PASS OS-real 4x4 matmul + mode latch");
